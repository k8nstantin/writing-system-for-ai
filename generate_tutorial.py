...
<div class="tutorial-panel">
  ...
  <div class="lesson-title">
    ...
    <span class="lesson-progress" id="lessonProgress">Rung 1/3</span>
  </div>
  <div class="instructions" id="lessonInstructions">
    In Alan, diamonds represent mental states, and circles represent entities. Let's write the core semantic thought: <strong>"I think"</strong>.
  </div>
...
<script>
  ...
  const lessons = [
    {
      ...
      validate: (line) => {
        ...
        // Element 0: think
        const isThink = elements[0].tagName.toLowerCase() === 'svg' && 
                        elements[0].parentNode.querySelector('.handle') && 
                        elements[0].parentNode.querySelector('.handle').textContent === 'think';
        // Element 1: spacer
        const isSpacer = elements[1].className === 'spacer';
        // Element 2: me
        const isMe = elements[2].tagName.toLowerCase() === 'svg' && 
                     elements[2].parentNode.querySelector('.handle') && 
                     elements[2].parentNode.querySelector('.handle').textContent === 'me';
                     
        return isThink && isSpacer && isMe;
      }
    },
    {
      ...
      validate: (out) => {
        ...
        const hasLiv = l0_el.length === 1 && l0_el[0].parentNode.querySelector('.handle').textContent === 'live';
        
        const l1_el = Array.from(lines[1].children).filter(el => el.id !== 'cursor');
        const hasDie = l1_el.length === 1 && l1_el[0].classList.contains('flipped-Y') && l1_el[0].parentNode.querySelector('.handle').textContent === 'live';
    ...
    },
    {
      ...
      validate: (out) => {
        ...
        const l0_el = Array.from(lines[0].children).filter(el => el.id !== 'cursor');
        const hasWant = l0_el.length === 1 && l0_el[0].parentNode.querySelector('.handle').textContent === 'want';
        const l0_ind = parseInt(lines[0].getAttribute('data-indent') || '0', 10) === 0;
        
        const l1_el = Array.from(lines[1].children).filter(el => el.id !== 'cursor');
        const hasYou = l1_el.length === 1 && l1_el[0].parentNode.querySelector('.handle').textContent === 'you';
  ...
  function highlightNextKey() {
    ...
    // Try to find by action or handle
  ...
  function checkLessonProgress() {
    const l = lessons[currentLessonIdx];
    // We pass both line or output depending on validator needs
  ...
  keys.forEach(key => {
    key.addEventListener('click', () => {
      ...
      // Track tutorial step incrementing
      ...
      if (svg) {
        ...
        // Add handle tag text beneath so our validation can read it
        if (handle) {
          const handleSpan = document.createElement('span');
          handleSpan.className = 'handle';
          handleSpan.style.display = 'none';
          handleSpan.textContent = handle;
          
          // Wrap SVG and handle span in a container to mirror key DOM structure
          const container = document.createElement('div');
          container.style.display = 'flex';
          container.style.alignItems = 'center';
          container.appendChild(clone);
          container.appendChild(handleSpan);
          activeLine.insertBefore(container, cursor);
        } else {
